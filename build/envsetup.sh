...
- pixelgerrit:     A Git wrapper that fetches/pushes patch from/to Evolution X Gerrit Review.
...
- repopick:        Utility to fetch changes from Evolution X Gerrit.
...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        if (adb shell getprop org.evolutionx.device | grep -q "$CUSTOM_BUILD"); then
...
function installboot()
{
    ...
    if (adb shell getprop org.evolutionx.device | grep -q "$CUSTOM_BUILD");
    then
...
function pixelgerrit() {
    ...
    local user=`git config --get review.gerrit.evolutionx.org.username`
...
function pixelrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "Evolution X Gerrit Rebase Usage: "
    ...
    git fetch "http://gerrit.evolutionx.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop org.evolutionx.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
